...
class SamplingProvider:
    ...
    Even with the GIL, this is helpful because the GIL is released while we're waiting for model predictions. We can use that time to
    fill the queues with the next batch of samples.
    ...
    crash if running >2 threads. For that reason, I wrote this util so we can fill (and block on) multiple deques with a single thread.
    """
        ...
        while getattr(t, 'do_run', True):
            ...
            if rnd_perlin is None:
                rnd_perlin = (create_perlin_noise(px=self.shape[0], color=self.perlin_color, batch_size=1,
                                                  normalize=True, precalc_fade=self.perlin_fade)[0], 20.)
    ...
    def get_perlin(self, return_stats=False):
        ...
        with self.cv_not_empty:
            ...
            item = self.queue_perlin.popleft()
            self.cv_not_full.notify()

        sample = item[0]
        stats = {"freq_exp": float(item[1])}                # Collect stats on image frequency.
        if return_stats:
            return sample, stats
        else:
            return sample
